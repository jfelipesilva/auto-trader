...
var users = {
    ...
    setOpenTrade: (user_id, strategy_id, pair, price) => {
        for(let i=(conf.users.length-1); i>=0; i--){
            if(conf.users[i].id == user_id){
                ...
                database.query("INSERT INTO orders (strategy_id, pair, user_id, priceFilled, type) VALUES ("+strategy_id+", '"+pair+"', "+user_id+", "+price+", 'buy')");
            }
        }
    },
    ...
    setCloseTrade: (user_id, strategy_id, pair, price, type) => {
        for(let i=(conf.users.length-1); i>=0; i--){
            if(conf.users[i].id == user_id){
                conf.users[i].strategyTrading = 0;
                database.query("UPDATE user SET strategyTrading = 0 WHERE id = "+user_id);
                database.query("INSERT INTO orders (strategy_id, pair, user_id, priceFilled, type) VALUES ("+strategy_id+", '"+pair+"', "+user_id+", "+price+", '"+type+"')");
            }
        }
    }
